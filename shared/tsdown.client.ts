/**
 * Shared tsdown preset for DSH UI plugin client bundles.
 *
 * Adapted from the official DeepSeek Harness checkout
 * (`packages/client/tsdown.client.ts`, MIT License,
 * https://github.com/deepseek-ai/deepseek-harness), plus the `libExternal`
 * option from the dsh-web-ui family-bucket adaptation (Apache-2.0,
 * https://github.com/zhu1090093659/dsh-web-ui). PLATFORM_MODULES is inlined
 * here (mirror of `@deepseek-ai/dsh-client-web/src/platform.ts`).
 *
 * Emits a closure-factory artifact: the bundle calls window.__ModuleLoader__.load
 * ({id, factory}) and resolves externals through the injected require (loader
 * module table — cordis DI entities, no globals, no import map). CSS Modules are
 * compiled by lightningcss inside the bundle: importing `x.module.css` yields the
 ...
 * (Mirror of the shell's seed table in `@deepseek-ai/dsh-client-web`.)
 */
 ...
 * shallowEqual) lives in runtime pending its promotion-time rehoming. At runtime
 * the lazy CJS table answers the require natively: runtime is an
 * immediately-tier row, its factory is registered before any dependent bundle
 * materializes. TODO(webload/store-rehome): remove with the store-engine
 ...
 * plus the browser client bundle. Client packages emit both halves in one run.
 * @param id - plugin id (package name), stamped into the __ModuleLoader__.load
 * handoff and onto the injected style tags.
 * @param libEntry - node-half entries, spelled at the call site so the
 * package-invariants gate can see `src/invariant.ts` in the package's own
 * tsdown.config.ts.
 * @param options - phase placement, lib overrides, extra node-half externals.
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    outputOptions: {
      ...
      // directories; sourcesContent keeps them usable without exposing that
